...
root.configure(background="#2C3C3F")
canvas = tk.Canvas(root, width=WIDTH, height=HEIGHT, background='#2C3C3F')
canvas.pack()
...
def update_clock():
    ...
    # Draw clock face
    canvas.create_oval(2, 2, WIDTH, HEIGHT, outline="#008B8B", width=2)

    # Draw hour numbers
    for i in range(12):
        angle = i * math.pi/6 - 1.57 #(math.pi/2=1.57)
    ...
    for i in range(60):
        ...
        if i % 5 == 0:
            canvas.create_line(x1, y1, x2, y2, fill="#00D2FF", width=3)
        else:
            canvas.create_line(x1, y1, x2, y2, fill="#00D2FF", width=1)

    # Draw hour hand
    hour_angle = (hour + minute/60) * math.pi/6 - math.pi/2
    hour_x = WIDTH/2 + 0.5 * WIDTH/2 * math.cos(hour_angle)
    hour_y = HEIGHT/2 + 0.5 * HEIGHT/2 * math.sin(hour_angle)
    canvas.create_line(WIDTH/2, HEIGHT/2, hour_x, hour_y, fill="#8A2BE2", width=6)
    ...
    canvas.create_line(WIDTH/2, HEIGHT/2, minute_x, minute_y, fill="#9400D3", width=4)

    # Draw second hand
    second_angle = second * math.pi/30 - math.pi/2
    second_x = WIDTH/2 + 0.6 * WIDTH/2 * math.cos(second_angle)
    second_y = HEIGHT/2 + 0.6 * WIDTH/2 * math.sin(second_angle)
    canvas.create_line(WIDTH/2, HEIGHT/2, second_x, second_y, fill="#4B0082", width=2)

    canvas.after(1000, update_clock)

date=dt.datetime.now()
label = tk.Label(root, text=f"{date:%A, %B %d, %Y}", font=('radioland', 17, 'bold'), background='#2C3C3F', foreground='#00D2FF')
label.pack(pady=20)
